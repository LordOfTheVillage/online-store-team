...
export const BankCard: React.FC<bankCardProps> = ({onChange, cardData}) => {
	...
	const changeCardNumber = (e: React.FormEvent<HTMLInputElement>) => {
		...
		setCardValue(cardValue.map(item => {if(item.id === 'cardNumber') item.value = cardNumber; return item}))
	...
	const changeCardDate = (e: React.FormEvent<HTMLInputElement>) => {
		...
		setCardValue(cardValue.map(item => {if(item.id === 'date') item.value = cardDate; return item}))
	}
  ...
  return <form className="card" onChange={handleCardData}>
		<div className="card__row">
			<div className={logo}></div>
			<PrimaryInput key={cardValue[0].id} 
									id={cardValue[0].id} 
									title={cardValue[0].title} 
									pattern={cardValue[0].pattern}
									value={cardValue[0].value} 
									onChange={(e: React.FormEvent<HTMLInputElement>) => changeCardNumber(e)}/></div>
		<div className="card__row">
		<PrimaryInput key={cardValue[1].id} 
									id={cardValue[1].id} 
		...
		<PrimaryInput key={cardValue[2].id} 
									id={cardValue[2].id} 
									title={cardValue[2].title} 
									pattern={cardValue[2].pattern}
									value={cardValue[2].value} 
									onChange={(e: React.FormEvent<HTMLInputElement>) => setCardValue(cardValue.map(item => {if(item.id === 'code') item.value = e.currentTarget.value; return item}))}/>
		</div>
	</form>
}
