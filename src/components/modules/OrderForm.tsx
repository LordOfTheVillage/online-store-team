...
import { FormInput } from '../simple/FormInput';
...
export const OrderForm: React.FC<OrderFormProps> = ({ onClose }) => {
  ...
  const handleSetCardData = (data: inputData[]) => {
    setCardValue(data);
  };

  const handleSetOrderFormValue = (data: React.FormEvent<HTMLInputElement>) =>
    setOrderFormValue(
      orderFormValue.map((item) => {
        if (item.id === data.currentTarget.id) item.value = data.currentTarget.value;
        return item;
      })
    );
  ...
  return (
    <div className="order-form">
      {orderFormValue.map((item) => (
        <div className="input-parent" key={item.id}>
          <FormInput
            id={item.id}
            title={item.title}
            type={item.type}
            ...
            value={item.value}
            onChange={handleSetOrderFormValue}
          />
        </div>
      ))}
      <BankCard onChange={handleSetCardData} cardData={cardData} />
      ...
      <ModalWindow onClose={closeModal} isOpenModal={isOpenModal}>
        <p className="order-complete">Order completed</p>
      </ModalWindow>
    </div>
  );
};
